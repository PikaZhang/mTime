<template>
    <div id="register">
        <div class="userInfo">
            <mu-text-field label="用户名" hintText="请输入用户名" type="text" @input="username" />
            <mu-text-field label="密码" hintText="6-20位字母、数字、符号组成" type="password" @input="password" />
            <mu-text-field label="昵称" hintText="请输入昵称" type="text" @input="name" />
        </div>
        <checklist label-position="right" @change.native="change" :options="commonList" required></checklist>
        <x-button @click.native="toRegister" type="primary" style="border-radius:99px;">
            免费注册
        </x-button>
...
export default {
    ...
    methods: {
        ...
        name(value){
           this.$store.commit('nicknameFn',value)
        },
        change(val) {
